...
> **Templates, not legal advice.** They are a proportionate starting point for a small
> UK brochure site. Anything collecting real personal data at scale, or trading as a
> registered company, needs a solicitor's eyes before launch.

## The four pages (fixed filenames — don't rename per build)

| Page | File | Why it's baseline |
|------|------|--------------------|
| Privacy Policy | `privacy.html` (or `/privacy` route) | UK GDPR requires it the moment a site collects any personal data — including via a contact form or analytics. |
| Cookie Policy | `cookies.html` (or `/cookies`) | PECR (Privacy and Electronic Communications Regulations) requires disclosure + consent for any non-essential cookie (analytics, marketing pixels). |
...
| Accessibility Statement | `accessibility.html` (or `/accessibility`) | Public-facing good practice under the Equality Act 2010 / WCAG; also documents the anti-slop a11y gates this workspace already enforces (pre-ship-gates.md §7), so it's mostly a truthful writeup of work already done. |
...
Write real copy for all four using the templates below. The honesty rule
(`copywriting.md` / anti-slop-rules §6) applies in full: **never invent** a
company registration number, ICO registration number, registered address, or
